...
// ⚠️ CHANGE THIS IF YOUR IP CHANGES
const BACKEND_URL = 'http://169.254.127.134:8080';
...
export default function Browse() {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          fetchAllItems(); // Fallback to all items
        }
      );
    } else {
      fetchAllItems();
  ...
  const fetchNearbyItems = (lat: number, lng: number) => {
  ...
  const fetchAllItems = () => {
    ...
    fetch(`${BACKEND_URL}/api/listings`) 
      ...
      .catch(err => {
        console.error("Fetch failed. Is your backend running on 169.254.127.134:8080?", err);
