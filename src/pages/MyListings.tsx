...
import { Calendar, Trash2, Loader2, Package, PlusCircle } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/listings/provider/${currentUser.id}`);
                ...
                <Card key={item.id} className="overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col h-full border-gray-200">
                  ...
                  <CardFooter className="px-6 py-4 border-t bg-gray-50/50">
                    <Button 
                        variant="destructive" 
                        size="sm" 
                        className="w-full hover:bg-red-600 shadow-none"
