...
export default function MyPayments() {
  ...
  const fetchMyRequests = () => {
    fetch(`http://localhost:8080/api/requests/borrower/${currentUser?.id}`)
      .then(res => res.json())
      .then(data => {
        const paymentRelated = data.filter((r: any) => r.status === 'ACCEPTED');
        // Sort newest first
        setRequests(paymentRelated.sort((a: any, b: any) => b.id - a.id));
        setLoading(false);
      });
  };
  ...
  const calculateTotal = () => {
    ...
    const dailyRate = 500; // Hardcoded for now. Update this if your DB has item daily rates!
    const deposit = selectedReq?.amount || 1000; // Using the amount from DB as deposit
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/requests/${selectedReq.id}/pay?userId=${currentUser?.id}&startDate=${startDate}&endDate=${endDate}&totalAmount=${totals.total}`, 
        { method: 'POST' }
      );
